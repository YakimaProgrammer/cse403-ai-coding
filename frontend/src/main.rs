...
fn app() -> Html {
                ...
                if let Some(file) = files.get(0) {
                    ...
                    read_as_text(&file, move |res| {
                        if let Ok(text) = res {
                            ...
                            columns.set(headers.iter().map(|s| s.to_string()).collect());
                            
                            let data: Vec<HashMap<String, String>> = reader
                                .deserialize()
                                .filter_map(|r| r.ok())
                                .collect();
                            csv_data.set(data);
    ...
    let on_solve = {
        ...
        Callback::from(move |_| {
            // Placeholder config - in a real app, these come from UI mapping state
